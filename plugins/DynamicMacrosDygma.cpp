...
namespace kaleidoscope
{
  namespace plugin
  {
    ...
    void DynamicMacros::readKeyCodeAndPlay(uint16_t pos, uint8_t flags, uint8_t keyStates, bool explicit_report)
    {
      Key key(p_macros_config->memory[pos++], // key_code
              flags);
    ...
    void DynamicMacros::updateDynamicMacroCache(void)
    {
      ...
      while (pos < sizeof(p_macros_config->memory))
      {
        macro = p_macros_config->memory[pos++];
        switch (macro)
        {
        ...
        case MACRO_ACTION_STEP_TAP_SEQUENCE:
        {
          ...
          do
          {
            flags = p_macros_config->memory[pos++];
            keyCode = p_macros_config->memory[pos++];
    ...
    void DynamicMacros::play(uint8_t macro_id)
    {
      ...
      while (true)
      {
        switch (macro = p_macros_config->memory[pos++])
        {
        case MACRO_ACTION_STEP_EXPLICIT_REPORT:
        ...
        case MACRO_ACTION_STEP_INTERVAL:
        {
          uint8_t inter1 = p_macros_config->memory[pos++];
          uint8_t inter2 = p_macros_config->memory[pos++];
          interval = (inter1 << 8) | inter2;
          break;
        }
        case MACRO_ACTION_STEP_WAIT:
        {
          uint8_t d2 = p_macros_config->memory[pos++];
          uint8_t d1 = p_macros_config->memory[pos++];
    ...
    EventHandlerResult DynamicMacros::onFocusEvent(const char *command)
    {
      ...
      if (strcmp_P(command + 7, PSTR("map")) == 0)
      {
        if (::Focus.isEOL())
        {
          for (uint16_t i = 0; i < sizeof(p_macros_config->memory); i++)
          {
            ::Focus.send(p_macros_config->memory[i]);
          }
        }
        else
        {
          ...
          while (!::Focus.isEOL())
          {
            ...
            cfgmem_macro_mem_save( pos++, &b, sizeof(b) );
          }
      ...
      if (strcmp_P(command + 7, PSTR("memory")) == 0)
      {
        if (::Focus.isEOL())
        {
          ::Focus.send(sizeof( p_macros_config->memory ));
        }
      }
    ...
    void DynamicMacros::reserve_storage( void )
    {
    ...
    void DynamicMacros::cfgmem_macro_mem_save( uint16_t mem_pos, uint8_t * p_data, uint16_t data_len )
    {
        result_t result = RESULT_ERR;

        if( ( mem_pos + data_len ) > sizeof( p_macros_config->memory ) )
        {
            ASSERT_DYGMA( false, "Macro memory space overflow" );
            return;
        }

        result = kbdfal_ll_memory_data_save( &p_macros_config->memory[mem_pos], p_data, data_len );
